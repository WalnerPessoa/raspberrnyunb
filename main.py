...
import uvicorn
from PIL import Image #walner 8/10/24
import numpy as np #walner 8/10/24
    ...
    <html>
        ...
        <body>
            <div class="container">
                ...
                <!-- Início do formulário para deletar o arquivo encodings.pkl -->
                <form action="/delete_encodings/" method="post" onsubmit="return confirm('Tem certeza que deseja deletar o arquivo encodings.pkl?')">
                    <input type="submit" value="Deletar Arquivo encodings.pkl">
                </form>
                <!-- Fim do formulário para deletar o arquivo encodings.pkl -->
...
async def upload(
    ...
    except Exception as e:#walner 8/10/24
        return HTMLResponse(content=f"""
            <html>
                <head>
                    ...
                    <script type="text/javascript">
                        function showPopup() {{
                            alert('Ocorreu um erro interno: {str(e)}');
                        }}
                    </script>   
                </head
                <body onload="showPopup()">
...
# Nova rota para deletar o arquivo encodings.pkl #walner 8/10/24
@app.post("/delete_encodings/")#walner 8/10/24
async def delete_encodings():#walner 8/10/24
    pickle_file = os.path.join(os.getcwd(), 'encodings.pkl')#walner 8/10/24

    # Verifica se o arquivo existe#walner 8/10/24
    if os.path.exists(pickle_file):#walner 8/10/24
        try:#walner 8/10/24
            os.remove(pickle_file)  # Tenta deletar o arquivo#walner 8/10/24
            return HTMLResponse(content="""
                <html>
                    <head>
                        <title>Deleção Concluída</title>
                        <script type="text/javascript">
                            // Função para exibir o alerta de confirmação e redirecionar
                            function showPopup() {
                                alert('Arquivo encodings.pkl foi deletado com sucesso! Por favor, reinicialize o sistema do Raspberry.');
                                window.location.href = '/';  // Redireciona para a página inicial após fechar o alerta
                            }
                        </script>
                    </head>
                    <body onload="showPopup()">
                        <h1>Deleção de Arquivo Concluída com Sucesso</h1>
                        <p>Por favor, reinicialize o sistema do Raspberry manualmente.</p>
                        <p>Você será redirecionado para a página inicial.</p>
                    </body>
                </html>
                """, status_code=200)

        except Exception as e:  # Captura qualquer erro que ocorrer#walner 8/10/24
            return {"error": f"Erro ao deletar o arquivo: {str(e)}"}  # Retorna a mensagem de erro#walner 8/10/24
    else:#walner 8/10/24
        return {"message": "Arquivo encodings.pkl não encontrado"}#walner 8/10/24


# Inicia o servidor FastAPI na porta 8000.
if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=8000)
    #uvicorn.run(app, host="0.0.0.0", port=8000, log_level="debug") #walner 8/10
